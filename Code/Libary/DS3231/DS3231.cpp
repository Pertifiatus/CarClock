...
DS3231::DS3231(SoftwareWire *theWire)
{
	_wire = theWire;
}
...
// ÄNDERUNG: const char* und const char* arrays
...
void DS3231::_burstRead()
{
	...
	_wire->requestFrom(DS3231_ADDR, 7);
	for (int i=0; i<7; i++)
	{
		if (_wire->available()) {
			_burstArray[i] = _wire->read();
		} else {
			_burstArray[i] = 0;
		}
	}
...
uint8_t DS3231::_readRegister(uint8_t reg)
{
	...
	_wire->requestFrom(DS3231_ADDR, 1);
	if (_wire->available()) {
		return _wire->read();
	}
	return 0;
